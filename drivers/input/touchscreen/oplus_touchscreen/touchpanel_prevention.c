...
static void record_latest_point(struct kernel_grip_info *grip_info,
				uint8_t index, struct point_info point)
{
	...
	if ((point.x == latest_point[cnt - 1].x)
			&& (point.y == latest_point[cnt - 1].y)) {
...
static bool large_area_judged_curved(struct kernel_grip_info *grip_info,
				     uint16_t *grip_side, struct point_info *points, int index)
{
...
static enum large_judge_status large_shape_judged_curved(
	struct kernel_grip_info *grip_info, uint16_t side, struct point_info *points,
	int index)
{
	struct point_info cur_p = points[index];
	struct curved_judge_para long_side_para = grip_info->curved_long_side_para;
	...
	if (((side >> TYPE_LONG_SIDE) & 0x01) || ((side >> TYPE_SHORT_SIDE) & 0x01)) {
		if (delta_time_ms <= grip_info->large_detect_time_ms) {
			TPD_DETAIL("id:%d, rx:%d, tx:%d, rx_er:%d, tx_er:%d. (%d %d)\n", index,
				   cur_p.rx_press, cur_p.tx_press, cur_p.rx_er, cur_p.tx_er, cur_p.x, cur_p.y);
				...
				if (cur_p.tx_press == 0) { /* long side */
					if (cur_p.rx_press >= long_side_para.large_palm_thd_1
						...
						grip_info->large_finger_status[index] = TYPE_HOLD_FINGER;
							...
							&& cur_p.rx_press <=
							long_side_para.normal_finger_thd_3) { /* finger need to exit right now */
						grip_info->large_finger_status[index] = TYPE_NORMAL_FINGER;
						judge_status = JUDGE_LARGE_TIMEOUT;
					}

				} else if (cur_p.rx_press != 0 && !(grip_info->touch_dir == VERTICAL_SCREEN
								    && grip_info->first_point[index].y <= grip_info->max_y /
								    2)) { /* long corner side */
					if (cur_p.rx_press >= long_side_para.large_palm_thd_1
							|| (cur_p.rx_er >= long_side_para.large_palm_thd_2 &&
							    delta_time_ms <= grip_info->large_detect_time_ms / 5)) {
				...
				if (cur_p.rx_press == 0) { /* short side */
					if (cur_p.tx_press >= short_side_para.large_palm_thd_1
						...
						grip_info->large_finger_status[index] = TYPE_EDGE_FINGER;
							...
							&& cur_p.tx_press <=
							short_side_para.normal_finger_thd_3) { /* finger need to exit right now */
						grip_info->large_finger_status[index] = TYPE_NORMAL_FINGER;
						judge_status = JUDGE_LARGE_TIMEOUT;
					}
...
static int acquire_matched_point(struct grip_point_info *point_array,
				 struct coord_buffer sp)
{
	int index = 0;
...
static void touch_report_work(struct work_struct *work)
{
	...
	if ((g_tp->grip_info->is_curved_screen || g_tp->grip_info->is_curved_screen_V2)
			&& g_tp->grip_info->sync_up_makeup[up_id]) {
		fiter_cnt = g_tp->grip_info->coord_filter_cnt;
		latest_point = g_tp->grip_info->latest_points[up_id];
		index_exp = acquire_matched_point(latest_point,
						  g_tp->grip_info->coord_buf[(up_id + 1) * fiter_cnt - 1]);
		...
		for (in = index_exp; in < POINT_DIFF_CNT; in++) {
			...
			if ((in < POINT_DIFF_CNT - 1) && (latest_point[in + 1].x == point_x)
					&& (latest_point[in + 1].y == point_y)) {	/*ignore same point */
				continue;
			}
...
static int curved_large_handle(struct kernel_grip_info *grip_info,
			       int obj_attention, struct point_info *points)
{
		...
		if (((obj_attention & TOUCH_BIT_CHECK) >> i) & 0x01) {	  /*finger down */
			...
			if (!(((grip_info->obj_prev_bit & TOUCH_BIT_CHECK) >> i) &
				...
				if (grip_info->large_point_status[i] ==
						UP_POINT) { /* if there is some point touch down, mark previous down points no need to make up point */
			...
			} else if (grip_info->makeup_cnt[i]
					&& MAKEUP_REAL_POINT != grip_info->makeup_cnt[i]) {
...
static void record_point_info(struct kernel_grip_info *grip_info,
			      point_info_type type, uint8_t index, struct point_info point)
{
	int in = 0, cnt = POINT_DIFF_CNT;
	struct grip_point_info *latest_point = NULL;
	...
	case TYPE_MAX_TX_POINT:
		if ((point.tx_press > grip_info->txMax_frame_point[index].tx_press)
				|| (0 == point.tx_press)) {
			grip_info->txMax_frame_point[index].x = point.x;
			grip_info->txMax_frame_point[index].y = point.y;
			grip_info->txMax_frame_point[index].tx_press = point.tx_press;
	...
	case TYPE_MAX_RX_POINT:
		if ((point.rx_press > grip_info->rxMax_frame_point[index].rx_press)
				|| (0 == point.rx_press)) {
			grip_info->rxMax_frame_point[index].x = point.x;
			grip_info->rxMax_frame_point[index].y = point.y;
			grip_info->rxMax_frame_point[index].tx_press = point.tx_press;
	...
	case TYPE_LATEST_POINT:
		...
		if ((point.x == latest_point[cnt - 1].x)
				&& (point.y == latest_point[cnt - 1].y)) {		 /*return when same point */
			return;
		}
...
static void init_latest_data(struct kernel_grip_info *grip_info, uint8_t index,
			     struct point_info point)
{
	return;	 /*return because of latest data have been inited in init_filter_data function */
}

static void start_makeup_timer(struct kernel_grip_info *grip_info,
			       uint8_t index)
{
	int ret = 0, up_id = 0;
	...
	hrtimer_start(&grip_info->grip_up_timer[index], ktime_set(0,
			grip_info->report_updelay_ms * 1000000), HRTIMER_MODE_REL);
}

static void mask_potential_mistouch(struct kernel_grip_info *grip_info,
				    struct point_info *points, int id)
{
	int index = 0;
	bool out_range = true;
	s64 delta_time_ms = 0;
	...
	uint16_t finger_status = 0,
		 debounce_ms = grip_info->large_reject_debounce_time_ms;
	...
	for (index = 0; index < TOUCH_MAX_NUM; index++) {
		...
		delta_time_ms = abs(grip_info->first_point[id].time_ms -
				    grip_info->first_point[index].time_ms);		 /*time interval */

		if ((POS_LONG_LEFT == pos) || (POS_LONG_RIGHT == pos)) {
			finger_status = TYPE_PALM_LONG_SIZE;
			out_range = (grip_info->first_point[index].y > min_val
				     && grip_info->first_point[index].y < max_val) ? false : true;

		} else if ((POS_SHORT_LEFT == pos) || (POS_SHORT_RIGHT == pos)) {
			finger_status = TYPE_PALM_SHORT_SIZE;
			out_range = (grip_info->first_point[index].x > min_val
				     && grip_info->first_point[index].x < max_val) ? false : true;
		}

		if ((delta_time_ms < debounce_ms) && !out_range
				&& (TYPE_REJECT_HOLD ==
				    grip_info->large_reject[index])) {	  /*already down id is under suspicious detect time */
...
static bool large_reject_covered(struct kernel_grip_info *grip_info,
				 struct point_info *points, int id)
{
	int index = 0;
	uint16_t min_val = 0, max_val = 0;
	...
	for (index = 0; index < TOUCH_MAX_NUM; index++) {
		if (UP_POINT == grip_info->large_point_status[index]) {
			if (grip_info->last_points_pos[index] != pos
					|| TYPE_REJECT_DONE !=
					grip_info->last_large_reject[index]) {			/*same point id must be handle here */
				continue;
			}

			under_time_interval = abs(grip_info->last_frame_point[index].time_ms -
						  grip_info->first_point[id].time_ms) <
					      debounce_ms;  /*new down id is under large up suspicious detect time */

		} else if (DOWN_POINT == grip_info->large_point_status[index]) {
			if (index == id || grip_info->points_pos[index] != pos
					|| TYPE_REJECT_DONE !=
					grip_info->large_reject[index]) {	   /*reject id must be different from judge id */
				continue;
			}
		...
		if ((POS_LONG_LEFT == pos) || (POS_LONG_RIGHT == pos)) {
			min_val = grip_info->rxMax_frame_point[index].y - (grip_info->max_y /
					grip_info->rx_num) * grip_info->rxMax_frame_point[index].rx_press / 2;
			max_val = grip_info->rxMax_frame_point[index].y + (grip_info->max_y /
					grip_info->rx_num) * grip_info->rxMax_frame_point[index].rx_press / 2;
			out_range = (cur_p.y > min_val && cur_p.y < max_val) ? false : true;

		} else if ((POS_SHORT_LEFT == pos) || (POS_SHORT_RIGHT == pos)) {
			min_val = grip_info->txMax_frame_point[index].x - (grip_info->max_x /
					grip_info->tx_num) * grip_info->txMax_frame_point[index].tx_press / 2;
			max_val = grip_info->txMax_frame_point[index].x + (grip_info->max_x /
					grip_info->tx_num) * grip_info->txMax_frame_point[index].tx_press / 2;
		...
		if (is_covered) {
			TPD_INFO("%s: id(%d) is included around the reject point(%d), status:%s.\n",
				 __func__, id, index, DOWN_POINT == grip_info->large_point_status[index] ? "down"
				 : "up");
			break;
		}
	}
...
static bool research_point_landed(struct kernel_grip_info *grip_info,
				  uint32_t research_pos_bits, struct point_info *points, int id)
{
	int index = 0;
	bool ret = false;
	uint8_t pos = grip_info->points_pos[id];
	...
	for (index = 0; index < TOUCH_MAX_NUM; index++) {
		...
		if ((0 == research_pos_bits)
				|| ((research_pos_bits >> grip_info->points_pos[index]) &
				    0x01)) {	   /*judge the pos we need */
			/* For horizontal, we just judge half the size */
			if ((POS_SHORT_LEFT == pos || POS_HORIZON_B_LEFT_CORNER == pos
					|| POS_HORIZON_T_LEFT_CORNER == pos)
					&& (points[index].y >= grip_info->max_y / 2)) {
				continue;
			}

			if ((POS_SHORT_RIGHT == pos || POS_HORIZON_B_RIGHT_CORNER == pos
					|| POS_HORIZON_T_RIGHT_CORNER == pos)
					&& (points[index].y < grip_info->max_y / 2)) {
				continue;
			}

			if (grip_info->points_pos[index] != pos
					&& (grip_info->first_point[index].time_ms > grip_info->first_point[id].time_ms -
					    debounce_ms)) {
				ret = true;
				TPD_INFO("%s: id(%d) meet soon down point(%d: %d %d).\n", __func__, id, index,
					 grip_info->first_point[index].x, grip_info->first_point[index].y);
				break;
			}
		}
	}
...
static uint8_t large_judge_pos(struct kernel_grip_info *grip_info, int index)
{
	...
	uint16_t width = grip_info->large_corner_width,
		 height = grip_info->large_corner_height;
	uint16_t top_width = grip_info->large_top_width,
		 top_height = grip_info->large_top_height;

	if (VERTICAL_SCREEN == grip_info->touch_dir) {
		if (fsp.y > grip_info->max_y -
				height) {					/*judge according to the range */
			if (fsp.x < width) {
		...
		if ((0 != fsp.tx_press) && (0 != fsp.rx_press)
				&& (fsp.y > grip_info->max_y /
				    2)) {		 /*judge according to the potential operation */
		...
		if (fsp.y <
				top_height) {					/*judge according to the press time and move status */
			if (fsp.x < top_width) {
		...
		if ((0 != fsp.tx_press) && (0 != fsp.rx_press)
				&& (fsp.y < grip_info->max_y / 2)) {		 /*top corner */
			if (fsp.x < grip_info->max_x / 2) {
		...
		if ((0 != fsp.tx_press) && (0 != fsp.rx_press)
				&& (fsp.x > grip_info->max_x / 2)) {		 /*top corner */
			if (fsp.y < grip_info->max_y / 2) {
	...
	} else if (LANDSCAPE_SCREEN_270 ==
			grip_info->touch_dir) {	  /*important message : pos is defined in horizontal 90 */
	...
	if ((0 == fsp.tx_press)
			&& (0 != fsp.rx_press)) {				   /*long side logic */
		pos = fsp.x < grip_info->max_x / 2 ? POS_LONG_LEFT : POS_LONG_RIGHT;

	} else if ((0 != fsp.tx_press) && (0 == fsp.rx_press)
			&& (VERTICAL_SCREEN != grip_info->touch_dir)) {			/*short side logic */
...
static uint8_t judge_center_down(struct kernel_grip_info *grip_info,
				 struct point_info *points, int id)
{
	int index = 0;
	...
	if (POS_CENTER_INNER == pos) {		  /*set already down point */
		for (index = 0; index < TOUCH_MAX_NUM; index++) {
			...
			if ((POS_SHORT_LEFT == tmp || POS_HORIZON_B_LEFT_CORNER == tmp
					|| POS_HORIZON_T_LEFT_CORNER == tmp)
					&& (points[id].y >= grip_info->max_y / 2)) {
				continue;
			}

			if ((POS_SHORT_RIGHT == tmp || POS_HORIZON_B_RIGHT_CORNER == tmp
					|| POS_HORIZON_T_RIGHT_CORNER == tmp)
					&& (points[id].y < grip_info->max_y / 2)) {
				continue;
			}
	...
	for (index = 0; index < TOUCH_MAX_NUM; index++) {
		...
		if (POS_CENTER_INNER ==
				grip_info->points_pos[index]) {							/*judge the pos we need */
...
static uint8_t corner_shape_matched(struct kernel_grip_info *grip_info,
				    struct point_info *points, int index)
{
	uint8_t ret = CORNER_SHAPE_NONE;
	struct point_info cur_p = points[index];
	uint8_t pos = grip_info->points_pos[index];
	uint16_t trx_sum_value = 0, x_width = grip_info->single_channel_x_len,
		 y_width = grip_info->single_channel_y_len;
	uint16_t trx_thd = grip_info->trx_reject_thd, rx_thd = grip_info->rx_reject_thd,
		 tx_thd = grip_info->tx_reject_thd;
	...
	if (POS_VERTICAL_LEFT_CORNER == pos || POS_VERTICAL_RIGHT_CORNER == pos) {
		if (cur_p.x > x_width && cur_p.x < grip_info->max_x - x_width) {
			if ((trx_sum_value >= rx_thd) && (trx_sum_value <= trx_thd)
					&& (cur_p.rx_press >= 2 * cur_p.tx_press)) {
	...
	if (cur_p.y > y_width && cur_p.y < grip_info->max_y - y_width) {
		if ((trx_sum_value >= tx_thd) && (trx_sum_value <= trx_thd)
				&& (cur_p.tx_press >= 2 * cur_p.rx_press)) {
			ret = CORNER_SHAPE_RATIO;
		}
...
static bool corner_exit_matched(struct kernel_grip_info *grip_info,
				struct point_info *points, int index)
{
	bool ret = false;
	struct point_info cur_p = points[index];
	...
	uint16_t xfsr_coupling_thd = grip_info->xfsr_corner_exit_thd,
		 yfsr_coupling_thd = grip_info->yfsr_corner_exit_thd,
		 exit_thd = grip_info->exit_match_thd;
	...
	} else if ((POS_VERTICAL_RIGHT_CORNER == pos)
			|| (POS_HORIZON_T_RIGHT_CORNER == pos)) {
	...
	if ((STATUS_CENTER_DOWN != grip_info->points_center_down[index])
		...
		if (max_distance > exit_dis) {
			TPD_INFO("%s: exit matched_0(%d) (%d %d %d %d %d %d)", __func__, index, cur_p.x,
				 cur_p.y, cur_p.tx_press, cur_p.rx_press, cur_p.tx_er, cur_p.rx_er);
	...
	if ((cur_p.rx_er * cur_p.rx_press <= xfsr_coupling_thd)
			&& (cur_p.tx_er * cur_p.tx_press <= yfsr_coupling_thd)) {
		TPD_INFO("%s: exit matched_1(%d) (%d %d %d %d %d %d)", __func__, index, cur_p.x,
			 cur_p.y, cur_p.tx_press, cur_p.rx_press, cur_p.tx_er, cur_p.rx_er);
		grip_info->exit_match_times[index]++;
	}
...
static bool top_exit_matched(struct kernel_grip_info *grip_info,
			     struct point_info *points, int index)
{
	bool ret = false;
	struct point_info cur_p = points[index];
	int16_t x_distance = 0, y_distance = 0, max_distance = 0;
	uint16_t exit_dis = grip_info->large_top_exit_distance,
		 exit_thd = grip_info->exit_match_thd;
	...
	if (max_distance > exit_dis) {
		TPD_INFO("%s: top exit matched(%d) (%d %d %d %d %d %d)", __func__, index,
			 cur_p.x, cur_p.y, cur_p.tx_press, cur_p.rx_press, cur_p.tx_er, cur_p.rx_er);
		grip_info->exit_match_times[index]++;
	}
...
static bool large_shape_matched(struct kernel_grip_info *grip_info,
				struct point_info *points, int index)
{
	struct point_info cur_p = points[index];
...
static bool large_exit_matched(struct kernel_grip_info *grip_info,
			       struct point_info *points, int index)
{
	bool ret = false;
	struct point_info cur_p = points[index];
	...
	uint16_t pos = grip_info->points_pos[index],
		 exit_thd = grip_info->exit_match_thd;
	uint16_t xfsr_coupling_thd = grip_info->xfsr_normal_exit_thd,
		 yfsr_coupling_thd = grip_info->yfsr_normal_exit_thd;
	uint16_t edge_narrow_witdh = grip_info->edge_swipe_narrow_witdh,
	...
	if ((cur_p.tx_er * cur_p.tx_press <= yfsr_coupling_thd)
			&& (cur_p.rx_er * cur_p.rx_press <= xfsr_coupling_thd)) {
		grip_info->exit_match_times[index]++;
		TPD_INFO("%s: exit matched(%d) (%d %d %d %d %d %d)", __func__, index, cur_p.x,
			 cur_p.y, cur_p.tx_press, cur_p.rx_press, cur_p.tx_er, cur_p.rx_er);
	}
	...
	if (STATUS_CENTER_DOWN != grip_info->points_center_down[index]) {
		if ((POS_LONG_LEFT == pos || POS_LONG_RIGHT == pos)
				...
				&& (TYPE_LONG_FINGER_HOLD != grip_info->large_finger_status[index])) {
			if (abs(cur_p.x - first_p.x) < edge_narrow_witdh
					&& abs(cur_p.y - first_p.y) > edge_exit_dis) {
				grip_info->exit_match_times[index]++;
				TPD_INFO("%s: exit matched_y(%d) (%d %d %d %d %d %d)", __func__, index, cur_p.x,
					 cur_p.y, cur_p.tx_press, cur_p.rx_press, cur_p.tx_er, cur_p.rx_er);
			}
		}

		if ((POS_SHORT_LEFT == pos || POS_SHORT_RIGHT == pos)
				&& (TYPE_PALM_SHORT_SIZE != grip_info->large_finger_status[index])) {
			if (abs(cur_p.y - first_p.y) < edge_narrow_witdh
					&& abs(cur_p.x - first_p.x) > edge_exit_dis) {
				grip_info->exit_match_times[index]++;
				TPD_INFO("%s: exit matched_x(%d) (%d %d %d %d %d %d)", __func__, index, cur_p.x,
					 cur_p.y, cur_p.tx_press, cur_p.rx_press, cur_p.tx_er, cur_p.rx_er);
			}
		}
	}
...
static bool finger_hold_matched(struct kernel_grip_info *grip_info,
				struct point_info *points, int id)
{
	int index = 0;
	...
	uint16_t coord_range = grip_info->max_y / grip_info->long_hold_divided_factor,
		 time_intval = grip_info->long_hold_debounce_time_ms;
	...
	for (index = 0; index < TOUCH_MAX_NUM; index++) {
		...
		if ((tmp_pos == pos) ||
			...
			under_range = abs(grip_info->first_point[index].y -
					  grip_info->first_point[id].y) < coord_range;
			under_time_interval = abs(grip_info->first_point[index].time_ms -
						  grip_info->first_point[id].time_ms) < time_intval;

			if (((TYPE_LONG_FINGER_HOLD == grip_info->large_finger_status[index])
					&& under_range) || under_time_interval) {
...
static enum large_judge_status large_shape_judged_V2(struct kernel_grip_info *grip_info, struct point_info *points, int index)
{
	...
	enum large_judge_status judge_status = JUDGE_LARGE_CONTINUE;
	s64 delta_time_ms = ktime_to_ms(ktime_get()) -
			    grip_info->first_point[index].time_ms;
	uint16_t long_stable_coupling_thd = grip_info->long_stable_coupling_thd,
		 short_stable_coupling_thd = grip_info->short_stable_coupling_thd;
	uint16_t x_coupling_result = 0, y_coupling_result = 0,
		 startx_coupling_result = 0, starty_coupling_result = 0,
		 secondx_coupling_result = 0, secondy_coupling_result = 0;

	/*caculate current coupling result */
	x_coupling_result = cur_p.rx_press * cur_p.rx_er;
	y_coupling_result = cur_p.tx_press * cur_p.tx_er;
	startx_coupling_result = grip_info->first_point[index].rx_press *
				 grip_info->first_point[index].rx_er;
	starty_coupling_result = grip_info->first_point[index].tx_press *
				 grip_info->first_point[index].tx_er;
	secondx_coupling_result = grip_info->second_point[index].rx_press *
				  grip_info->second_point[index].rx_er;
	secondy_coupling_result = grip_info->second_point[index].tx_press *
				  grip_info->second_point[index].tx_er;
	...
	if ((POS_LONG_LEFT == pos) || (POS_LONG_RIGHT == pos)) {
		...
		*/
		...
		if (x_coupling_result == grip_info->last_frame_point[index].rx_er *
				grip_info->last_frame_point[index].rx_press) {
			grip_info->fsr_stable_time[index] += ktime_to_ms(ktime_get()) -
							     grip_info->last_frame_point[index].time_ms;		 /*record point stable time */
		...
		if (grip_info->fsr_stable_time[index] > grip_info->fsr_stable_time_thd) {
			...
			if ((abs(startx_coupling_result - secondx_coupling_result) <
					grip_info->long_hold_maxfsr_gap) &&
					(startx_coupling_result > grip_info->long_start_coupling_thd) &&
					(startx_coupling_result - x_coupling_result >
					 grip_info->long_hold_changed_thd)) {
				judge_status = JUDGE_LARGE_OK;
				grip_info->large_finger_status[index] = TYPE_LONG_FINGER_HOLD;
		...
		/*judge the stable status */
		if (y_coupling_result == grip_info->last_frame_point[index].tx_er *
				grip_info->last_frame_point[index].tx_press) {
			grip_info->fsr_stable_time[index] += ktime_to_ms(ktime_get()) -
							     grip_info->last_frame_point[index].time_ms;		 /*record point stable time */
		...
		if (grip_info->fsr_stable_time[index] > grip_info->fsr_stable_time_thd) {
			...
			if ((abs(starty_coupling_result - secondy_coupling_result) <
					grip_info->short_hold_maxfsr_gap) &&
					(starty_coupling_result > grip_info->short_stable_coupling_thd) &
					(starty_coupling_result - y_coupling_result >
					 grip_info->short_hold_changed_thd)) {
				judge_status = JUDGE_LARGE_OK;
	...
	} else if ((POS_VERTICAL_LEFT_CORNER == pos)
			|| (POS_VERTICAL_RIGHT_CORNER == pos) ||
		...
		if ((pos != POS_VERTICAL_LEFT_CORNER) && (pos != POS_VERTICAL_RIGHT_CORNER)) {
			if ((STATUS_CENTER_DOWN == grip_info->points_center_down[index])
					&& (TYPE_REJECT_DONE == grip_info->last_large_reject[index])
					&& (pos == grip_info->last_points_pos[index])) {
				judge_status = JUDGE_LARGE_OK;
		...
		if ((y_coupling_result == grip_info->last_frame_point[index].tx_er *
				grip_info->last_frame_point[index].tx_press) &&
				(x_coupling_result == grip_info->last_frame_point[index].rx_er *
				 grip_info->last_frame_point[index].rx_press)) {
			grip_info->fsr_stable_time[index] += ktime_to_ms(ktime_get()) -
							     grip_info->last_frame_point[index].time_ms;		 /*record point stable time */

			if (grip_info->fsr_stable_time[index] > grip_info->fsr_stable_time_thd) {
				if ((starty_coupling_result <= grip_info->yfsr_corner_exit_thd)
						&& (startx_coupling_result <= grip_info->xfsr_corner_exit_thd) &&
						(y_coupling_result <= grip_info->yfsr_corner_exit_thd)
						&& (x_coupling_result <=
						    grip_info->xfsr_corner_exit_thd)) {					/*free long press */
					judge_status = JUDGE_LARGE_TIMEOUT;
		...
		if ((delta_time_ms > grip_info->large_corner_detect_time_ms)
				&& (grip_info->fsr_stable_time[index] > grip_info->fsr_stable_time_thd)) {
			if ((x_coupling_result > grip_info->long_stable_coupling_thd)
					|| (y_coupling_result > grip_info->short_stable_coupling_thd)) {
				judge_status = JUDGE_LARGE_OK;
...
static bool large_area_judged_V2(struct kernel_grip_info *grip_info,
				 struct point_info *points, int index)
{
	bool result = false;
	...
	if ((POS_LONG_LEFT == pos) || (POS_LONG_RIGHT == pos)) {
		if (large_shape_matched(grip_info, points, index)) {
			if ((TYPE_REJECT_DONE == grip_info->large_reject[index])
					&& (TYPE_PALM_LONG_SIZE != grip_info->large_finger_status[index])) {
				TPD_INFO("%s: long shape matched(%d) (%d %d %d %d %d %d)", __func__, index,
					 cur_p.x, cur_p.y, cur_p.tx_press, cur_p.rx_press, cur_p.tx_er, cur_p.rx_er);
				grip_info->large_finger_status[index] = TYPE_PALM_LONG_SIZE;
			}
		...
		if (large_shape_matched(grip_info, points, index)) {
			if ((TYPE_REJECT_DONE == grip_info->large_reject[index])
					&& (TYPE_PALM_SHORT_SIZE != grip_info->large_finger_status[index])) {
				TPD_INFO("%s: short shape matched(%d) (%d %d %d %d %d %d)", __func__, index,
					 cur_p.x, cur_p.y, cur_p.tx_press, cur_p.rx_press, cur_p.tx_er, cur_p.rx_er);
				grip_info->large_finger_status[index] = TYPE_PALM_SHORT_SIZE;
			}
...
static bool touchup_judged_V2(struct kernel_grip_info *grip_info, int index)
{
	...
	uint16_t long_hold_maxfsr_gap = grip_info->long_hold_maxfsr_gap,
		 short_hold_maxfsr_gap = grip_info->short_hold_maxfsr_gap;
	s64 delta_time_ms = ktime_to_ms(ktime_get()) -
			    grip_info->first_point[index].time_ms;
	...
	if ((POS_LONG_LEFT == pos) || (POS_LONG_RIGHT == pos)) {
		...
		if (((abs(startx_coupling_result - secondx_coupling_result) <
				long_hold_maxfsr_gap) && (startx_coupling_result > long_start_coupling_thd)) ||
		...
		if (((abs(starty_coupling_result - secondy_coupling_result) <
				short_hold_maxfsr_gap) && (starty_coupling_result > short_start_coupling_thd))
				||
...
static int curved_large_handle_V2(struct kernel_grip_info *grip_info,
				  int obj_attention, struct point_info *points)
{
	int m_index = 0;
		...
		if (((obj_attention & TOUCH_BIT_CHECK) >> m_index) & 0x01) {	  /*finger down */
			...
			if (grip_info->large_out_status[m_index]) {
				if (MAKEUP_REAL_POINT != grip_info->makeup_cnt[m_index]) {
					record_point_info(grip_info, TYPE_LATEST_POINT, m_index,
							  points[m_index]);			  /*record latest different points */
				}

				if (grip_info->makeup_cnt[m_index] >
						0) {												   /*means we need to continue to make up to the real point */
					if (grip_info->makeup_cnt[m_index] <= fiter_cnt) {
						...
						TPD_INFO("id:%d makeup :%d times.(%d %d)\n", m_index,
							 grip_info->makeup_cnt[m_index], points[m_index].x, points[m_index].y);
			...
			if (1 == grip_info->frame_cnt[m_index]) {						/*init when first touch down */
				grip_info->large_point_status[m_index] =
					DOWN_POINT;						 /*set down status */
				...
				grip_info->points_pos[m_index] = large_judge_pos(grip_info,
								 m_index);							   /*judge the position of each id */
				grip_info->points_center_down[m_index] = judge_center_down(grip_info, points,
						m_index);			 /*judge whether we have center point down already */

			} else if (2 ==
					grip_info->frame_cnt[m_index]) {								  /*record second frame point info */
				record_point_info(grip_info, TYPE_SECOND_POINT, m_index, points[m_index]);
			}

			grip_info->point_unmoved[m_index] = (abs(points[m_index].x -
							     grip_info->first_point[m_index].x) <= 0 &&
							     abs(points[m_index].y - grip_info->first_point[m_index].y) <=
							     0);	 /*record move status */
			record_point_info(grip_info, TYPE_LATEST_POINT, m_index,
					  points[m_index]);			  /*record latest different points */
			record_point_info(grip_info, TYPE_MAX_TX_POINT, m_index,
					  points[m_index]);			  /*record points info of max tx channel */
			record_point_info(grip_info, TYPE_MAX_RX_POINT, m_index,
					  points[m_index]);			  /*record points info of max rx channel */
			...
			if (exit_status) {
				...
				if (!grip_info->point_unmoved[m_index]) {	/*means once in large judge area */
					...
					TPD_INFO("id:%d makeup m:%d times.(%d %d)(%d %d %d %d)\n", m_index,
						 grip_info->makeup_cnt[m_index], points[m_index].x, points[m_index].y,
						 points[m_index].tx_press, points[m_index].rx_press, points[m_index].tx_er,
						 points[m_index].rx_er);
				}

			} else if (TYPE_REJECT_DONE == grip_info->large_reject[m_index]) {
				obj_final = obj_final & (~(1 <<
							   m_index));				/*if already reject, just mask it */

			} else {
				TPD_DETAIL("id:%d, rx:%d, tx:%d, rx_er:%d(%d), tx_er:%d(%d). (%d %d)\n",
					   m_index, points[m_index].rx_press, points[m_index].tx_press,
					   points[m_index].rx_er, points[m_index].rx_er * points[m_index].rx_press,
					   points[m_index].tx_er, points[m_index].tx_er * points[m_index].tx_press,
					   points[m_index].x, points[m_index].y);
				judge_state = large_shape_judged_V2(grip_info, points, m_index);

				if (JUDGE_LARGE_OK == judge_state) {
					obj_final = obj_final & (~(1 <<
								   m_index));				/*reject for large shape */
					grip_info->large_reject[m_index] = TYPE_REJECT_DONE;

					if ((TYPE_PALM_LONG_SIZE == grip_info->large_finger_status[m_index])
							|| (TYPE_PALM_SHORT_SIZE == grip_info->large_finger_status[m_index])) {
						mask_potential_mistouch(grip_info, points,
									m_index);	  /*mask other seperate reported point around the large shape */
					}
					...
					obj_final = obj_final & (~(1 <<
								   m_index));			/*reject for continue detect */
			...
			if (grip_info->large_point_status[m_index] == DOWN_POINT) {
				TPD_DETAIL("up id(%d) status: %d, %d, %d, %d.\n", m_index,
					   grip_info->points_pos[m_index], grip_info->points_center_down[m_index],
					   grip_info->large_out_status[m_index], grip_info->large_reject[m_index]);
			}

			if (grip_info->large_out_status[m_index]) {	   /*already exit the grip status */
				if (grip_info->makeup_cnt[m_index] > 0
						&& MAKEUP_REAL_POINT != grip_info->makeup_cnt[m_index]) {
				...
				grip_info->last_large_reject[m_index] =
					TYPE_REJECT_NONE;				   /*update last status, should not update this id while no touch down */
				grip_info->last_points_pos[m_index] =
					grip_info->points_pos[m_index];	   /*update last status, should not update this id while no touch down */
				record_point_info(grip_info, TYPE_LAST_POINT, m_index,
						  points[m_index]);	/*update last status, should not update this id while no touch down */
				...
				grip_info->last_large_reject[m_index] =
					TYPE_REJECT_HOLD;				   /*update last status, should not update this id while no touch down */
				grip_info->last_points_pos[m_index] =
					grip_info->points_pos[m_index];	   /*update last status, should not update this id while no touch down */
				record_point_info(grip_info, TYPE_LAST_POINT, m_index,
						  points[m_index]);	/*update last status, should not update this id while no touch down */
				...
				grip_info->last_large_reject[m_index] =
					TYPE_REJECT_DONE;				   /*update last status, should not update this id while no touch down */
				grip_info->last_points_pos[m_index] =
					grip_info->points_pos[m_index];	   /*update last status, should not update this id while no touch down */
				record_point_info(grip_info, TYPE_LAST_POINT, m_index,
						  points[m_index]);	/*update last status, should not update this id while no touch down */
...
static void init_para_addr(struct kernel_grip_info *grip_info)
{
	...
	TPD_INFO("%s index(%d) is %s the range.\n", __func__, index,
		 index > KEY_ADDR_NUMS ? "over" : "under");
}
			   ...
			   struct kernel_grip_info *grip_info)
{
	...
	if (grip_info->is_curved_screen_V2) {
		...
		for (in = 0; in < KEY_ADDR_NUMS; in++) {
			seq_printf(s, "%s: %d\n", key_addr_arrays[in].name,
				   key_addr_arrays[in].addr != NULL ? * (key_addr_arrays[in].addr) : -1);
		}
	...
	list_for_each(pos, &grip_info->large_zone_list) {
		...
		if (grip_info->is_curved_screen) {
			if (strstr(grip_area->name, "curved")) {
				seq_printf(s,
					   "name:%24s, start_point:(%4d, %4d), width:(%4d, %4d), exit_thd:%4d, exit_tx_er:%4d, exit_rx_er:%4d, support_dir:0x%02x, side:0x%02x.\n",
					   grip_area->name, grip_area->start_x, grip_area->start_y, grip_area->x_width,
					   grip_area->y_width, grip_area->exit_thd,
...
large_ver_corner_width:%4d, large_hor_corner_width:%4d, large_corner_distance:%4d\n",
	...
	if (grip_info->is_curved_screen) {
		...
		seq_printf(s, "edge_finger_thd:%4d, hold_finger_thd:%4d, normal_finger_thd_1:%4d, normal_finger_thd_2:%4d, normal_finger_thd_3:%4d\n\
			   ...
			   long_side_para->normal_finger_thd_1,
			   long_side_para->normal_finger_thd_2, long_side_para->normal_finger_thd_3,
			   long_side_para->large_palm_thd_1, long_side_para->large_palm_thd_2,
			   long_side_para->palm_thd_1, long_side_para->palm_thd_2,
			   long_side_para->small_palm_thd_1, long_side_para->small_palm_thd_2);
		seq_printf(s, "curved_large_judge_para_for_short_side:\n");
		seq_printf(s, "edge_finger_thd:%4d, hold_finger_thd:%4d, normal_finger_thd_1:%4d, normal_finger_thd_2:%4d, normal_finger_thd_3:%4d\n\
						large_palm_thd_1:%4d, large_palm_thd_2:%4d, palm_thd_1:%4d, palm_thd_2:%4d, small_palm_thd_1:%4d, small_palm_thd_2:%4d\n",
			   short_side_para->edge_finger_thd, short_side_para->hold_finger_thd,
			   short_side_para->normal_finger_thd_1,
			   short_side_para->normal_finger_thd_2, short_side_para->normal_finger_thd_3,
			   short_side_para->large_palm_thd_1, short_side_para->large_palm_thd_2,
			   short_side_para->palm_thd_1, short_side_para->palm_thd_2,
			   short_side_para->small_palm_thd_1, short_side_para->small_palm_thd_2);
		seq_printf(s, "\n");
	}
...
static int kernel_grip_init_V2(struct kernel_grip_info *grip_info,
			       struct device *dev)
{
	...
	cnt = of_property_count_elems_of_size(dev->of_node,
					      "prevention,makeup_cnt_weight", sizeof(uint32_t));
	...
	ret = of_property_read_u32_array(dev->of_node, "prevention,updelay_time_ms",
					 temp_array, 1);
	...
	ret = of_property_read_u32_array(dev->of_node, "prevention,large_corner_range",
					 temp_array, 2);
	...
	ret = of_property_read_u32_array(dev->of_node,
					 "prevention,large_corner_judge_condition", temp_array, 5);
	...
	ret = of_property_read_u32_array(dev->of_node,
					 "prevention,trx_reject_condition", temp_array, 3);
	...
	ret = of_property_read_u32_array(dev->of_node, "prevention,large_judge_time_ms",
					 temp_array, 3);
	...
	ret = of_property_read_u32_array(dev->of_node,
					 "prevention,large_exit_condition", temp_array, 3);
	...
	ret = of_property_read_u32_array(dev->of_node,
					 "prevention,single_channel_width", temp_array, 2);
	...
	ret = of_property_read_u32_array(dev->of_node,
					 "prevention,normal_tap_condition", temp_array, 2);
	...
	ret = of_property_read_u32_array(dev->of_node, "prevention,long_edge_condition",
					 temp_array, 4);
	...
	ret = of_property_read_u32_array(dev->of_node,
					 "prevention,long_fingerhold_condition", temp_array, 4);

	if (ret) {
		grip_info->long_hold_debounce_time_ms = 100;
		grip_info->long_hold_divided_factor =
			6;	/*default value, no need to change it */
	...
	ret = of_property_read_u32_array(dev->of_node,
					 "prevention,short_edge_condition", temp_array, 4);
	...
	ret = of_property_read_u32_array(dev->of_node, "prevention,top_corner_config",
					 temp_array, 3);
	...
	ret = of_property_read_u32_array(dev->of_node, "prevention,edge_swipe_config",
					 temp_array, 2);
	...
	ret = of_property_read_u32_array(dev->of_node,
					 "prevention,trx_strict_reject_condition", temp_array, 3);
	...
	ret = of_property_read_u32_array(dev->of_node,
					 "prevention,long_strict_edge_condition", temp_array, 2);
	...
	ret = of_property_read_u32_array(dev->of_node,
					 "prevention,short_strict_edge_condition", temp_array, 2);
	...
	ret = of_property_read_u32_array(dev->of_node,
					 "prevention,large_strict_exit_condition", temp_array, 2);
	...
	ret = of_property_read_u32_array(dev->of_node,
					 "prevention,corner_move_rejected", temp_array, 1);
	...
	if (grip_info->coord_buf) {
		for (i_index = 0; i_index < TOUCH_MAX_NUM; i_index++) {
			for (j_index = 0; j_index < grip_info->coord_filter_cnt; j_index++) {
				grip_info->coord_buf[i_index * grip_info->coord_filter_cnt + j_index].weight =
					makeup_para[j_index + 1];
			}
		}
	...
	for (i_index = 0; i_index < TOUCH_MAX_NUM; i_index++) {
		hrtimer_init(&grip_info->grip_up_timer[i_index], CLOCK_MONOTONIC,
			     HRTIMER_MODE_REL);
...
struct kernel_grip_info *kernel_grip_init(struct device *dev)
{
	...
	int temp_array[2] = {0}, cond_width[4] = {0}, large_width[2] = {0},
			    large_corner_width[3] = {0}, eli_width[6] = {0}, curved_large_width[5] = {0};
	struct grip_zone_area *grip_zone = NULL;
	...
	grip_info->is_curved_screen = of_property_read_bool(dev->of_node,
	...
	grip_info->is_curved_screen_V2 = of_property_read_bool(dev->of_node,
	...
	if (grip_info->is_curved_screen) {
		ret = of_property_read_u32_array(dev->of_node,
						 "prevention,large_curved_long_judge_para", long_judge_para, 11);
		...
		ret = of_property_read_u32_array(dev->of_node,
						 "prevention,large_curved_short_judge_para", short_judge_para, 11);
		...
		ret = of_property_read_u32_array(dev->of_node,
						 "prevention,curved_large_area_width", curved_large_width, 5);
		...
		ret = of_property_read_u32_array(dev->of_node,
						 "prevention,grip_large_detect_time", temp_array, 1);
		...
		ret = of_property_read_u32_array(dev->of_node,
						 "prevention,grip_down_delta_time", temp_array, 1);
