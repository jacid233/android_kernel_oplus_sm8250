...
static void kalman_handle(struct touchpanel_data *ts, int obj_attention, struct point_info *points)
{
    ...
    for (i = 0; i < ts->max_num; i++) {
        ...
        if (((obj_attention & TOUCH_BIT_CHECK) >> i) & 0x01 && (points[i].status != 0)) {
            ...
            KalmanFilter(points[i].x, &point_buf->kal_x_last.x, &point_buf->kal_p_last.x, point_buf->delta_time, algo_info->kalman_info);
            KalmanFilter(points[i].y, &point_buf->kal_x_last.y, &point_buf->kal_p_last.y, point_buf->delta_time, algo_info->kalman_info);
...
void switch_kalman_fun(struct touchpanel_data *ts, bool game_switch)
{
    ...
    if (game_switch) {
        if (CHK_BIT(ts->algo_info->kalman_info.enable_bit, KALMAN_GAME_BIT)) {
        ...
        if (CHK_BIT(ts->algo_info->kalman_info.enable_bit,(1 << ts->smooth_level))) {
            ts->algo_info->enable_kalman = true;
        } else {
            ts->algo_info->enable_kalman = false;
        }
...
static ssize_t proc_kalman_value_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
    ...
    snprintf(page, PROC_MAX_BUF, "%u,%u,%u,%u,0x%08x.\n",
