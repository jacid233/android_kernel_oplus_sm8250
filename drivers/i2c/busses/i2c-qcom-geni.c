...
const char *chg_i2c = "i2c@994000";
static int chg_i2c_err = 0;
static int pre_chg_i2c_err = 0;

int oppo_get_chg_i2c_err(void)
{
	printk(KERN_ERR "%s: [%d]\n", __func__, chg_i2c_err);
	return chg_i2c_err;
}
EXPORT_SYMBOL(oppo_get_chg_i2c_err);

void oppo_clear_chg_i2c_err(void)
{
	chg_i2c_err = 0;
	printk(KERN_ERR "%s\n", __func__);
	return;
}
EXPORT_SYMBOL(oppo_clear_chg_i2c_err);
#endif /*VENDOR_EDIT*/
...
#ifdef VENDOR_EDIT
/*Jianchao.Shi@PSW.BSP.CHG.Basic, 2020/03/30, sjc Add for mp2650 i2c error*/
		if (!strcmp(gi2c->adap.dev.of_node->full_name, chg_i2c)
				&& msgs[i].addr == CHG_DEVICE_ADDR
				&& gi2c->err) {
			chg_i2c_err = 1;
			//dev_info(gi2c->dev, "chg_i2c_err[%d]\n", chg_i2c_err);
		}
		if (chg_i2c_err ^ pre_chg_i2c_err) {
			pre_chg_i2c_err = chg_i2c_err;
			dev_info(gi2c->dev, "chg_i2c_err[%d]\n", chg_i2c_err);
		}
#endif /*VENDOR_EDIT*/
