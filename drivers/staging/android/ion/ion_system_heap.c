				    ...
				    unsigned long flags)
{
	...
	if (boost_pool) {
		while (size_remaining > 0) {
			...
			if (!info)
				break;
			if (boost_pool->usage == ION_FLAG_GRAPHIC_BUFFER) {
				buffer->flags |= ION_FLAG_GRAPHIC_BUFFER;
			}
...
struct ion_heap *ion_system_heap_create(struct ion_platform_heap *data)
{
	...
	if (!IS_ERR(boost_root_dir)) {
		heap->gr_pool = boost_pool_create(heap, ION_FLAG_GRAPHIC_BUFFER,
						 256 * 256,
						  boost_root_dir, "graphic");
		if (!heap->gr_pool)
			pr_err("%s: create boost_pool graphic failed!\n",
			       __func__);
