...
static bool gr_pool_skip_shrink = false;
static int gr_pool_should_protect = 0;
static bool gr_pool_not_to_shrink(struct ion_boost_pool *boost_pool)
{
	return boost_pool->usage == ION_FLAG_GRAPHIC_BUFFER &&
		gr_pool_skip_shrink;
}

static bool gr_pool_is_full(struct ion_boost_pool *boost_pool)
{
	return boost_pool_nr_pages(boost_pool) > gr_pool_should_protect;
}
		    ...
		    int order)
{
	...
	if (pool->usage == ION_FLAG_GRAPHIC_BUFFER) {
		if (boost_pool_nr_pages(pool) < gr_pool_should_protect) {
			ion_page_pool_free(pool->pools[order_to_index(order)], page);
			return 0;
		}
	} else if (boost_pool_nr_pages(pool) < MAX_POOL_SIZE) {
		ion_page_pool_free(pool->pools[order_to_index(order)], page);
		return 0;
	}
		      ...
		      int nr_to_scan)
{
	...
	if (gr_pool_not_to_shrink(boost_pool)) {
		if (gr_pool_is_full(boost_pool)) {
			int pool_max_shrink = boost_pool_nr_pages(boost_pool) - gr_pool_should_protect;
			if (nr_to_scan != 0)
				nr_to_scan = max(nr_to_scan, pool_max_shrink);
			else {
				     ...
				     size_t count, loff_t *ppos)
{
	...
	if (boost_pool->usage & ION_FLAG_CAMERA_BUFFER) {
		...
		kcrit_scene_flag = 1;
		gr_pool_skip_shrink = true;
		gr_pool_should_protect = nr_pages;
		wake_up_interruptible(&kcrit_scene_wait);
		boost_pool->high = nr_pages;
		boost_pool_wakeup_process(boost_pool);
	}
