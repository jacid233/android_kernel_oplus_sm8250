...
//Qiujin.Pan@Network.Data,2020/10/24,add for IPA dump

bool ipa3_check_hw_idle(void)
{
	struct ipahal_ipa_state_reg ipa_state;
	u32 reg_val;

	if (!ipahal_is_init())
		return true;
	IPADBG("checking hw idle before disable clk\n");
	memset(&ipa_state, 0, sizeof(struct ipahal_ipa_state_reg));
	/* only call from ipa_disable_clk so no need ipa-clk check */
	reg_val = ipahal_read_reg(IPA_STATE);
	memcpy(&ipa_state, &reg_val, sizeof(u32));
	if (likely(ipa_state.rx_idle && ipa_state.tx_idle &&
		ipa_state.ipa_rx_hps_empty &&
		ipa_state.ipa_hps_dps_empty &&
		ipa_state.ipa_dps_tx_empty &&
		ipa_state.bam_gsi_idle && ipa_state.ipa_noc_idle &&
		ipa_state.ipa_rsrc_mngr_db_empty &&
		ipa_state.ipa_ackmngr_db_empty))
		return true;
	return false;
}
