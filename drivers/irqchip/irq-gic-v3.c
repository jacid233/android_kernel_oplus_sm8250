...
#define PCIE_RC2_IRQ_MIN 200
#define PCIE_RC2_IRQ_MAX 300
...
atomic_t pcie_rc2_first_msg_ipa = ATOMIC_INIT(0);
#endif /* VENDOR_EDIT */
...
/* Jianchao.Shi@PSW.BSP.Power.Basic, 2019/12/10, sjc Add for PCIE irq power debug */
		if (irq >= 211 && irq <= 242)//pcie2
			name = MODEM_IRQ_NAME;
		if (irq >= 142 && irq <= 173)//pcie0
			name = WLAN_DATA_IRQ_NAME;
#endif /*VENDOR_EDIT*/
...
/* Jianchao.Shi@PSW.BSP.Power.Basic, 2019/12/10, sjc Add for PCIE irq power debug */
#ifdef CONFIG_MHI_DEBUG
			if (!strncmp(name, "qcommsi", 7))
				g_msi_power[0][0] = 0x88;
#endif
#endif /*VENDOR_EDIT*/
...
//add modem for pcie debug msg
			if (irq >= PCIE_RC2_IRQ_MIN && irq < PCIE_RC2_IRQ_MAX) {
				if (atomic_read(&pcie_rc2_first_msg_qmi) == 0)
					atomic_set(&pcie_rc2_first_msg_qmi, 1);
				if (atomic_read(&pcie_rc2_first_msg_ipa) == 0)
					atomic_set(&pcie_rc2_first_msg_ipa, 1);
			}
			// add modem for ipcc debug msg
			if(strncmp(name, IPCC_1, sizeof(IPCC_1)-1) == 0) {
				if ((irq < PCIE_RC2_IRQ_MIN || irq >= PCIE_RC2_IRQ_MAX) && (atomic_read(&pcie_rc2_first_msg_qmi) == 0))
					atomic_set(&pcie_rc2_first_msg_qmi, 1);
