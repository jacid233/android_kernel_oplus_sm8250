...
int upload_mm_fb_kevent(enum OPLUS_MM_DIRVER_FB_EVENT_MODULE module, unsigned int event_id, unsigned char *payload) {
	...
	if (OPLUS_MM_DIRVER_FB_EVENT_TO_DISPLAY == module) {
		memcpy(user_msg_info->tag, DP_FB_EVENT, sizeof(DP_FB_EVENT));
	} else {
	...
	if ((OPLUS_DISPLAY_EVENTID_DRIVER_ERR == event_id) ||
			(OPLUS_DISPLAY_EVENTID_GPU_FAULT == event_id)) {
		memcpy(user_msg_info->tag, ATLAS_FB_EVENT, sizeof(ATLAS_FB_EVENT));
		printk(KERN_INFO "mm_kevent fb:send to atlas\n");
		mm_fb_kevent_send_to_user(user_msg_info);
	}
