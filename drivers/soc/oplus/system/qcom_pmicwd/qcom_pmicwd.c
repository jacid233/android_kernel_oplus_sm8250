...
** Version : 1.0
** Date : 2020/04/03
** Author : ZhangFei@BSP.KERNEL.STABILITY
**
** ------------------------------- Revision History: -----------
**  <author>        <date>        <version >        <desc>
**   ZhangFei        2020/04/02    1.0               Build this moudle
******************************************************************/
...
static int pmicwd_test_set(void *data, u64 val)
{
	...
	if(val){
		...
		if(pon->wd_task){
			kthread_stop(pon->wd_task);
			qpnp_pon_wd_config(1);  // wd will be disabled in wd_task stop flow, so re-enabled 
			pon->wd_task = NULL;
