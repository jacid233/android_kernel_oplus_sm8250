...
#ifdef OPLUS_FEATURE_UIFIRST
	if (test_task_ux(tsk) && delta_ms > UX_IO_WAIT_TIMEOUT)	//first decide ux iowait
...
static int get_free_mem(char **data)
{
	...
	/* Free memory = xxx MB. */
	free_mem =
	    (int)(global_zone_page_state(NR_FREE_PAGES) >> (20 - PAGE_SHIFT));
	return free_mem;
}
...
static unsigned char io_queue_index = 0;
...
static int get_blk_nsync_io_distribution(char **data)
{
	...
	ret = sprintf(nsyncio_buff,
		      ...
		      req_para.req_block_para.
		      total_delay[IO_MONITOR_DELAY_NSYNC],
		      ...
		      req_para.req_block_para.
		      stage_ten[IO_MONITOR_DELAY_NSYNC]);
...
static int task_status_get_value(char **data)
{
	...
	memcpy(flow_taskio_bak, submit_taskio,
	       sizeof(struct task_io_info) * PID_LENGTH);
	for (i = 0; i < PID_LENGTH; i++) {
		taskio = flow_taskio_bak + i;
		if (taskio->pid)
			len += snprintf(flow_freq_buffer + len,
				     TASK_STATUS_LENGTH - len,
				     "\n%d %d %d %d %llu %llu %llu",
				     taskio->pid, taskio->tgid, taskio->uid,
				     taskio->state, taskio->all_read_bytes,
				     taskio->all_write_bytes, taskio->fsync);
	}
...
static int get_task_status(char **data)
{
	...
	for (i = 0; i < PID_LENGTH; i++) {
		...
		if (taskio->pid)
			len += snprintf(freq_buffer + len,
				     TASK_STATUS_LENGTH - len,
				     "%d %s %d %d %llu %llu %llu\n",
				     taskio->pid, taskio->comm, taskio->tgid,
				     taskio->uid, taskio->read_bytes,
				     taskio->write_bytes, taskio->fsync);
	}
	*data = freq_buffer;
	return len;
}
...
static int io_monitor_show(struct seq_file *m, void *v)
{
	...
	while (p != &daily_list) {
		...
		} else if (STR_ENTRY_TYPE == entry->type
			   || STR_FREE_ENTRY_TYPE == entry->type) {
