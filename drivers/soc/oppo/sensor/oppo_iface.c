			   ...
			   const char *buff, size_t count)
{
	...
	if (sscanf(buff, "%d", &brightness) == 1) {
		brightness = brightness < 0 ? (brightness_pre < 0 ? 0 : brightness_pre) : brightness;
		g_the_chip->brightness = brightness;
		//printk("[oppo_brightness_store] brightness %d brightness_pre %d",brightness,brightness_pre);
		if (brightness_pre != brightness) {
			brightness_pre = brightness;
			g_the_chip->is_need_to_report = true;
			//printk("[oppo_brightness_store] is_need_to_report %d", g_the_chip->is_need_to_report);
			   ...
			   const char *buff, size_t count)
{
	...
	if (sscanf(buff, "%d", &dc_actived) == 1) {
		dc_actived = dc_actived < 0 ? (dc_actived_pre < 0 ? 0 : dc_actived_pre) : dc_actived;
