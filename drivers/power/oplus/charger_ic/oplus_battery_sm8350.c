...
int haptic_error_status = 2;
int pmk_error_status;
...
#define HAPTIC_REG1_HIGH BIT(7)
#define HAPTIC_REG_VALUE 0x00

int oplus_get_haptic_status(void)
{
	int rc = 0;
	struct battery_chg_dev *bcdev = NULL;
	struct psy_state *pst = NULL;
	...
	rc = read_property_id(bcdev, pst, BATT_GET_REG1_STATUS);
	if (rc) {
		chg_err("get BATT_GET_REG1_STATUS fail, rc=%d\n", rc);
	}

	spmi_play = (pst->prop[BATT_GET_REG1_STATUS]) & HAPTIC_REG1_HIGH;

	rc = read_property_id(bcdev, pst, BATT_GET_REG2_STATUS);
	if (rc) {
		chg_err("get BATT_GET_REG2_STATUS fail, rc=%d\n", rc);
	}

	if(!spmi_play && ((pst->prop[BATT_GET_REG2_STATUS]) == HAPTIC_REG_VALUE)){
		rc = read_property_id(bcdev, pst, BATT_GET_HAPTIC_STATUS);
			if (rc) {
				chg_err("get BATT_GET_HAPTIC_STATUS fail, rc=%d\n", rc);
			}
			haptic_error_status = pst->prop[BATT_GET_HAPTIC_STATUS];
			chg_err("get BATT_GET_HAPTIC_STATUS success, haptic_error_status=%d\n",haptic_error_status);
	}

	chg_err("!!!oplus haptic_status=%d reg1=%d reg2=%d spmi_play=%d\n", haptic_error_status,pst->prop[BATT_GET_REG1_STATUS],pst->prop[BATT_GET_REG2_STATUS],spmi_play);

	return haptic_error_status;
}

int oplus_get_haptic_error_status(void)
{
	return haptic_error_status;
}

EXPORT_SYMBOL(oplus_get_haptic_error_status);
...
static void oplus_pmk_trig_work(struct work_struct *work)
{
	int pmk_status = 0;
	struct oplus_chg_chip *chip = g_oplus_chip;
	...
	pmk_status = oplus_get_pmk_status();
}

static void oplus_haptic_trig_work(struct work_struct *work)
{
	int haptic_status = 0;
	struct oplus_chg_chip *chip = g_oplus_chip;

	if (chip == NULL) {
		pr_err("[haptic] chip null, return\n");
		return;
	}

	haptic_status = oplus_get_haptic_status();
}
...
int oplus_get_pmk_status(void)
{
	...
	rc = read_property_id(bcdev, pst, BATT_GET_PMK_STATUS);
	if (rc) {
		chg_err("set bcc current fail, rc=%d\n", rc);
		return rc;
	}

	pmk_status = pst->prop[BATT_GET_PMK_STATUS];
	pmk_error_status = pmk_status;

	chg_err("!!!oplus pmk_status=%d status: %d\n", pmk_status, BATT_GET_PMK_STATUS);

	return pmk_status;
}

int oplus_get_pmk_error_status(void)
{
	return pmk_error_status;
}

EXPORT_SYMBOL(oplus_get_pmk_error_status);
...
static int battery_chg_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&bcdev->vchg_pmk_work, oplus_pmk_trig_work);
	INIT_DELAYED_WORK(&bcdev->vchg_haptic_work, oplus_haptic_trig_work);
...
#endif
	schedule_delayed_work(&bcdev->vchg_pmk_work, msecs_to_jiffies(3000));
	schedule_delayed_work(&bcdev->vchg_haptic_work, msecs_to_jiffies(3000));
	return 0;
error:
