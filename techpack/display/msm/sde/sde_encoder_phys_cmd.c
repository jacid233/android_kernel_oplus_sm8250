		...
		ktime_t profile_timestamp)
{
	...
	if (ret == -ETIMEDOUT) {
		...
		#ifdef VENDOR_EDIT
		/*Mark.Yao@PSW.MM.Display.LCD.Stable,2019-12-15 add wr_ptr_irq irq kevent data */
		{
			unsigned char payload[150] = "";

			scnprintf(payload, sizeof(payload), "NULL$$EventID@@%d$$wr_ptr_irq_timeout@@%d %d",
					 OPPO_MM_DIRVER_FB_EVENT_ID_ESD, switch_te, oppo_dimlayer_fingerprint_failcount);
