...
/* Xiong.Hu@MULTIMEDIA.AUDIODRIVER.CODEC, 2020/09/05, CR2731147 avoid overflow during swr fifo read/write */
...
static irqreturn_t swr_mstr_interrupt_v2(int irq, void *dev)
{
	...
	if (swrm_request_hw_vote(swrm, LPASS_HW_CORE, true)) {
		...
		/*CHUNYU.XIE@MULTIMEDIA.AUDIODRIVER.MBHC 2020.11.14 add for necessary log*/
		dev_err(swrm->dev, "%s hw_core failed to enable\n", __func__);
		#endif
		ret = IRQ_NONE;
		goto exit;
	}
	...
	for (i = 0; i < SWRM_INTERRUPT_MAX; i++) {
		...
		case SWRM_INTERRUPT_STATUS_SLAVE_PEND_IRQ:
			...
			dev_dbg(swrm->dev, "%s: Trigger irq to slave device\n",
				__func__);
			#else
			dev_err(swrm->dev, "%s: Trigger irq to slave device\n",
				__func__);
			#endif
			status = swr_master_read(swrm, SWRM_MCP_SLV_STATUS);
			ret = swrm_find_alert_slave(swrm, status, &devnum);
