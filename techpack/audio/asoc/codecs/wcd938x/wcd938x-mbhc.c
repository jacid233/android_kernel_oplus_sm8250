...
static void wcd938x_read_irq_status(struct wcd_mbhc *mbhc)
{
	struct snd_soc_component *component = mbhc->component;
	pr_err("%s: irq status = %x\n",__func__, snd_soc_component_read32(component,
...
#ifdef OPLUS_ARCH_EXTENDS
    /*CHUNYU.XIE@MULTIMEDIA.AUDIODRIVER.MBHC 2020.11.14 add for necessary log*/
	.read_irq_status = wcd938x_read_irq_status,
#endif
};
