...
//Asiga@PSW.NW.DATA.2120730, 2019/06/26.
//Add for classify glink wakeup services.
	if (atomic_read(&ipa_wakeup_hook_boot) == 1 && (atomic_read(&ipa_first_msg) == 1 || atomic_read(&pcie_rc2_first_msg_ipa) == 1)) {
		if (atomic_read(&ipa_first_msg) == 1) {
			atomic_set(&ipa_first_msg, 2);
		} else {
			atomic_set(&pcie_rc2_first_msg_ipa, 2);
		}
