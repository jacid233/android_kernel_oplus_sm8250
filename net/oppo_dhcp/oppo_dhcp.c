...
static int handle_dhcp_packet(struct sock *sk, struct sk_buff *skb, struct net_device *dev, struct packet_type *pt) {
	...
	if (ext_len >= 4 ) {
		...
		case DHCPACK:
			...
			// notify offers
			if (oppo_dhcp_get_notify_state(dev) && mt == DHCPOFFER) {
...
static int oppo_dhcp_netlink_init(void)
{
	...
	oppo_dhcp_sock = netlink_kernel_create(&init_net, NETLINK_OPPO_DHCP, &cfg);
...
{
	oppo_dhcp_hooks_table_hrd = register_net_sysctl(&init_net, "net/oplus_dhcp_hooks",
